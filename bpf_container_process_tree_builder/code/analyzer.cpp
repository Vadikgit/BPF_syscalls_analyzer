...
const std::string arg_specify_container_to_catch_syscalls = "-container_name";
...
void process_arguments(int argc, char **argv, struct analyzer_bpf *skel)
{
  ...
  { // analyzer started
    ...
    pid_t to_track_pid_val = 0;

    bpf_map_update_elem(to_trace_value_map_fd, &to_track_pid_key, &to_track_pid_val, 0);
    //
